...
if [ -z "${start_idx}" ] || [ -z "${batch_size}" ]; then
    echo "Usage: bash step3.1_completion_multiagent_virtual.sh <input_file> <start_idx> <batch_size> [model_path] [base_url] [api_key] [mcp_server_dir] [max_workers] [timeout]"
    exit 1
fi
...
compute_expected_output_file() {
    ...
    config_str="${model_abbreviation}_multiagent_pfc"
...
ensure_base_url_ready() {
    ...
    echo "Endpoint ${normalized_url} is unavailable. Starting local vLLM for ${vllm_model_path}..."
...
if [ -f "${expected_output_file}" ]; then
    if [[ "$(is_all_error_output "${expected_output_file}")" == "all_error" ]]; then
        echo "Existing shard output is all errors. Regenerating: ${expected_output_file}"
    else
