...
This runbook tests the O*NET task-first pipeline through step 3.1 on a fixed 100-prompt smoke sample.
...
- Use endpoint `http://fs-mbz-gpu-758:8000/v1`, API key `EMPTY`, and model `kimi-k2.5`.
...
- Run commands from the repo root:
  ```bash
  cd /mnt/weka/home/yuekai.sun/Toucan/datagen
  ```
- Activate the Python environment:
  ```bash
  conda activate toucan
  ```
- Ensure the endpoint `http://fs-mbz-gpu-758:8000/v1` is reachable for step 1.2 and step 3.1.
...
bash -n step1.2_onet_completion.sh
bash -n step3.1_completion_agent.sh1
```

Set the smoke-test runtime variables:

```bash
export SMOKE_BASE_URL="http://fs-mbz-gpu-758:8000/v1"
export SMOKE_API_KEY="EMPTY"
export SMOKE_MODEL="kimi-k2.5"
```
...
```bash
python step1.1_gen_questions_from_onet_tasks.py \
  --num_tasks 3 \
  --total_prompts 100 \
  --self_contained \
  --no_refs \
  --output_folder ../data \
  --job_name self_contained_three_onet_tasks_q_no_refs_smoke
```
...
Do not use `step1.2_onet_completion.sh` for this smoke test unless you specifically want ranged shard output. That wrapper requires `start_idx` and `batch_size`.
