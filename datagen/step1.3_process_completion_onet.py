...
def get_args():
    # Experiment Settings
    parser = argparse.ArgumentParser(description="Tool Use Question Processing and Sanitization Manager (onet mode only).")
...
print(f"Tool Use Question Processing and Sanitization Manager (onet mode only).\nArguments:\n{args}") # For logging
...
def filter_metadata_by_target_tools(metadata, target_tools_str):
    ...
    Args:
        ...
        target_tools_str: Comma-separated string of target tool names (format: "server_name::tool_name")
    ...
    # Only process with :: case (standard format for onet mode)
...
def sanitize_questions(input_file, sanitized_output, distance_output, preview_distance_file, preview_sanitized_file, sentence_model, encoding_batch_size, distance_threshold, search_space_size, search_batch_size, device):
    ...
    # Only use new field names
    analyses = [item.get('tool_analysis', '') for item in dataset_items]
    ...
    with open(distance_output, 'w', encoding='utf-8') as f_out:
        for idx in tqdm(range(len(questions)), desc="Preparing all entries"):
            ...
            # Add analysis field (only new field name: tool_analysis)
            if idx < len(analyses) and analyses[idx]:
                entry['tool_analysis'] = analyses[idx]

            # Add workflow field if it exists (only new field name: cross_tool_workflow)
...
def print_processing_summary(stats):
    """
    Print a summary of the processing results for onet mode.
    """
    print("\n" + "="*60)
    ...
    print(f"onet Questions: {stats['onet']}")
    ...
    if stats['onet'] > 0:
        ...
        if stats['server_count_distribution']:
            ...
            for count, freq in sorted(stats['server_count_distribution'].items()):
                percentage = (freq / stats['onet']) * 100
                print(f"  {count} servers: {freq} questions ({percentage:.1f}%)")

    print("="*60 + "\n")

def main():
    print(f"Tool Use Question Processing Pipeline (onet mode only). Arguments: {args}")
