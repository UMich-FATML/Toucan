#!/bin/bash
#
# Step 1.2 (Agent Version): Run agent-based completions for question generation with REAL MCP tool execution.
#
# This script uses the OpenAI Agent framework with REAL MCP servers (not virtual tools).
# The model will actually execute the tools during generation to validate they work.
#
# Usage: bash step1.2_completion_agent.sh <input_file> [model_path] [engine] [step]
#
...
if [ -z "$input_file" ]; then
    ...
    echo "Usage: bash step1.2_completion_agent.sh <input_file> [model_path] [engine] [step]"
    exit 1
fi
...
if [ "$engine" == "vllm_api" ]; then
    if curl -s http://localhost:8000/v1/models > /dev/null 2>&1; then
        echo -e "${GREEN}[step1.2_agent] Found existing vLLM server on port 8000, reusing it.${NC}"
    else
        echo -e "${BLUE}[step1.2_agent] No vLLM server found, starting one...${NC}"
        ...
        echo -e "${GREEN}[step1.2_agent] vLLM server started with PID: ${VLLM_PID}${NC}"

        # Clean up the server we started on exit
        cleanup() {
            echo "[step1.2_agent] Cleaning up vLLM server (PID: $VLLM_PID)..."
